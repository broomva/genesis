...
import { parseLine, parseStream, sessionIdOf, textBlocks, toolUses, } from "./parser";
...
describe("parser", () => {
    ...
    test("extracts session_id from any carrier", () => {
        expect(sessionIdOf(parseLine('{"type":"system","session_id":"s1"}'))).toBe("s1");
        expect(sessionIdOf(parseLine('{"type":"result","session_id":"s2"}'))).toBe("s2");
    });
...
describe("projection reducer — RunPhase state machine", () => {
    ...
    test("a tool_result after awaiting clears the gate back to running", () => {
        const awaiting = reduceAll(fixture("awaiting.ndjson"));
